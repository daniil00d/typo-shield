...
export type Every<bools extends boolean[]> = bools[number] extends true ? true
  : false;
...
type KebabToCamel<str> = str extends `${infer first}-${infer rest}` ? `${first}${KebabToCamel<Capitalize<rest>>}`
  : str;

type SnakeToCamel<str> = str extends `${infer first}_${infer rest}` ? `${first}${SnakeToCamel<Capitalize<rest>>}`
  : str;
...
> = str extends `${infer first}${infer rest}` ? first extends UppercaseLetter ? output extends '' ? SnakeCase<rest, Lowercase<first>>
...
export type KebabCase<
  str,
  output extends string = '',
> = str extends `${infer first}${infer rest}` ? first extends UppercaseLetter ? output extends '' ? KebabCase<rest, Lowercase<first>>
...
export type IsArrayStrict<a> = a extends readonly any[] ? Not<IsTuple<a>>
  : false;
