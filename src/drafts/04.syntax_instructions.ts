...
type GetDirectives<I extends Record<string, any>> = I extends { meta: string[] }
  ? I["meta"] extends [infer First extends string, ...infer Other]
    ? First extends `#includes $${infer DirectiveName extends string}`
      ? Omit<I, "meta"> & { directive: DirectiveName; meta: Other }
      : I
    : never
  : never;
...
type GetURLParameters<URL extends string> =
  URL extends `${infer smt extends string}/{${infer Name extends string}: ${infer Type extends ParameterType}}`
    ? { name: Name; type: Type extends "Number" ? number : string }
    : undefined;

type GetURL<I extends Record<string, any>> = I extends { meta: string[] }
  ? I["meta"] extends [infer First extends string, ...infer Other]
    ? First extends `@ ${infer URL extends string}`
      ? Omit<I, "meta"> & { url: URL; meta: Other } & {
...
> = I extends { meta: string[] }
  ? I["meta"] extends [infer First extends string, ...infer Other]
    ? First extends `> json::body::${infer InputSchemeName extends string}`
      ? Omit<I, "meta"> & { input: _Schema["input"]; meta: Other }
      : I
    : never
  : never;
...
> = I extends { meta: string[] }
  ? I["meta"] extends [infer First extends string, ...infer Other]
    ? First extends `< json::output::${infer InputSchemeName extends string}`
      ? Omit<I, "meta"> & { output: _Schema["output"]; meta: Other }
      : I
    : never
  : never;
